...
TURNING_TO_GARAGE = 1.2 #TURNING_TO_GARAGE; the higher the number the more turn
...
MIN_AREA = 400
TURN_RIGHT = 60.0
CIRCLE_ANGLE  = 360.0  # degrees to sweep around ball

# Circle P-controller gains
KP_HEADING    = 1.2    # heading correction gain
KD_DIST       = 0.3    # radial distance correction gain
...
def drive(direction, slow):
    if slow == 1:
        if direction == 0:
            ...
            turtle.cmd_velocity(linear=0.1, angular=direction * 0.2)
    else:
        if direction == 0:
...
def get_ball_world_position(odometry):
    """
    ...
    bx = rx + STOP_DISTANCE * math.cos(rtheta)
    by = ry + STOP_DISTANCE * math.sin(rtheta)
...
def parking(l1, l2):
    ...
    while True:
        ...
        if state == 1:  # Turning to center
            duration = get_duration((math.pi / 2 - side * angle) * 2, 0.4)
            if elapsed < duration:
                print("Turning to center", side)
                turtle.cmd_velocity(linear=0.0, angular=-side * 0.4)
            else:
                state = 2
                start_time = now  # reset času pro další fázi

        elif state == 2:  # Driving to center
            duration = get_duration(abs(garage_center_distance(l1, l2)), 0.2 * DRIVING_TO_GARAGE_AXIS)
            if elapsed < duration:
            ...
            duration = get_duration((math.pi / 2) * TURNING_TO_GARAGE, 0.4)
            if elapsed < duration:
                print("Turning to garage")
                turtle.cmd_velocity(linear=0.0, angular=side * 0.4)
            else:
                state = 4
                start_time = now
            ...
            depth = get_depth_at(SCREEN_CENTER_X, SCREEN_CENTER_Y)
...
def main():
    ...
    global start_position, get_ball_location
    ...
    while not turtle.is_shutting_down():
        ...
        # ------------------------------------------------------------------ #
            ...
            # TODO: Maybe more sophisticated search algorithm than just turning around?
            if idx is None:
            ...
            if depth is not None and depth <= STOP_DISTANCE:
                ...
                ball_world_x, ball_world_y = get_ball_world_position(position)
        ...
        # ------------------------------------------------------------------ #
            ...
            dist_error   = current_dist - STOP_DISTANCE  # +ve → too far out
            ...
            angular = KP_HEADING * heading_error - KD_DIST * dist_error
